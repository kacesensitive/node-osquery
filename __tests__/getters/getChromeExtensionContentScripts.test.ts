...
describe('getChromeExtensionContentScripts', () => {
    it('resolves with chrome_extension_content_scripts table on success', async () => {
        ...
        await expect(getChromeExtensionContentScripts()).resolves.toEqual(expectedChromeExtensionContentScripts);
        expect(mockRunOsquery).toHaveBeenCalledWith('SELECT * FROM chrome_extension_content_scripts', [OsType.LINUX, OsType.DARWIN, OsType.WINDOWS_NT]);
    });
    ...
    it('rejects with error on command failure', async () => {
        ...
        await expect(getChromeExtensionContentScripts()).rejects.toEqual('Command failed');
    });
});
